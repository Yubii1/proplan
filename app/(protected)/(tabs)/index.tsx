...
type Stat = { id: string; title: string; value: number; icon: 'briefcase-outline' | 'list-outline' | 'alarm-outline' | 'checkmark-done-outline' };
type ProjectRow = { id: string; status?: string | null; due_date?: string | null; title?: string | null; progress?: number | null };
...
export default function Home() {
  ...
  const load = useCallback(async () => {
      ...
      if (!error && data) {
        ...
        console.log('Home load: fetched projects', rows.length, rows.map(p => ({ id: p.id, status: p.status, due_date: p.due_date, title: p.title, user_id: (p as any).user_id })));
        const active = rows.filter(p => (p.status ?? 'draft') !== 'completed').length;
        const completed = rows.filter(p => (p.status ?? 'draft') === 'completed').length;
        const dueThisWeek = rows.filter(p => p.due_date && isDueThisWeek(new Date(p.due_date))).length;
        setStats([
          { id: '1', title: 'Active Projects', value: active, icon: 'briefcase-outline' },
          // Tasks Today not yet implemented; keep slot for UI consistency
          { id: '2', title: 'Tasks Today', value: 0, icon: 'list-outline' },
          { id: '3', title: 'Due This Week', value: dueThisWeek, icon: 'alarm-outline' },
          { id: '4', title: 'Completed', value: completed, icon: 'checkmark-done-outline' },
        ]);
        ...
        const upcoming = rows
          ...
          .map(p => ({
            ...
            date: p.due_date ? formatShort(parseDateOnly(p.due_date as string)) : 'No deadline',
          }));
        ...
        const progressValues = rows.map(p => {
          ...
          const derived = status === 'completed' ? 1 : status === 'pending' ? 0.5 : 0;
          const hasProgress = typeof p.progress === 'number' ? p.progress / 100 : undefined;
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: spacing.xl }}>
        ...
        <FlatList
          ...
          keyExtractor={(i) => i.id}
          renderItem={({ item }) => <StatCard title={item.title} value={item.value} icon={item.icon} />}
        />
        ...
        <View style={styles.quickActions}>
          <TouchableOpacity style={styles.quickBtn} onPress={() => router.push('/(protected)/addProjects')}>
            <Ionicons name="add-circle-outline" size={22} color={colors.primary} />
            <Text style={styles.quickBtnText}>Add Project</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.quickBtn} onPress={() => router.push('/(protected)/(tabs)/calendar')}>
            <Ionicons name="calendar-outline" size={22} color={colors.primary} />
            <Text style={styles.quickBtnText}>Calendar</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.quickBtn} onPress={() => router.push('/(protected)/(tabs)/progress')}>
        ...
        <TouchableOpacity style={styles.aiCard} onPress={() => router.push('/(protected)/(tabs)/progress')}>
          <Ionicons name="sparkles-outline" size={24} color={colors.primary} />
          <View style={{ marginLeft: spacing.sm }}>
            <Text style={styles.aiCardTitle}>Boost your productivity</Text>
            <Text style={styles.aiCardSubtitle}>
              “You have 3 tasks that can be completed today to reach 85% of your goal.”
            </Text>
          </View>
        </TouchableOpacity>
        ...
        <View style={styles.deadlineList}>
          {deadlines.length === 0 ? (
            <Text style={{ color: colors.textSecondary }}>No upcoming deadlines</Text>
          ) : (
            deadlines.map((d, idx) => (
              <TouchableOpacity key={idx} style={styles.deadlineItem} onPress={() => router.push('/(protected)/(tabs)/calendar')}>
      ...
      <FAB.Group
        ...
        actions={[
          { icon: 'plus', label: 'Add Project', onPress: () => router.push('/(protected)/addProjects') },
          { icon: 'calendar', label: 'Calendar', onPress: () => router.push('/(protected)/(tabs)/calendar') },
        ]}
